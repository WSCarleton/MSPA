...
for i = 1 : nmodes
    ...
    title(strcat('Hx (TE mode: ', num2str(i),' )')); xlabel('x'); ylabel('y'); 
    %for v = edges, line(v{:}); end


    subplot(122);
    contourmode(x,y,Hy(:,:,i));
    title(strcat('Hy (TE mode: ', num2str(i),' )')); xlabel('x'); ylabel('y'); 
    %for v = edges, line(v{:}); end
...
for j = 1:nmodes
    ...
    title(strcat('Hx (TM mode: ', num2str(j),' )')); xlabel('x'); ylabel('y'); 
    %for v = edges, line(v{:}); end


    subplot(122);
    contourmode(x,y,Hy(:,:,j));
    title(strcat('Hy (TM mode: ', num2str(j),' )')); xlabel('x'); ylabel('y'); 
    %for v = edges, line(v{:}); end
end
